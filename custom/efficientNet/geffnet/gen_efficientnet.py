...
class GenEfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=6, num_features=1280, stem_size=32, fix_stem=False,
        ...
        if in_chans == 6:
            print("LiDAR")
            self.conv_stem = select_conv2d(in_chans, stem_size, 3, stride=(1, 2), padding=pad_type)
        else:
            print("Camera")
...
def _gen_efficientnet(variant, channel_multiplier=1.0, depth_multiplier=1.0, pretrained=False,
                      in_channels=6, **kwargs):
    ...
    if se_flag == False:
        arch_def = [[('_').join(block[0].split('_')[:-1]), ] for block in arch_def]
    ...
    with layer_config_kwargs(kwargs):
        ...
        # only adapt if range image backbone
        if in_channels == 6:
            # change channel size to lidar channel size
            for key, value in model_kwargs.items():
                if key == 'in_chans' and value == 3:
                    model_kwargs[key] = in_channels
                    # print(model_kwargs[key])
                # check sub arrays for stride
                # adapt stride to only downsample width
                if isinstance(value, list):
                    print(value[1][0]['stride'])

                    if value[1][0]['stride'] == 2:
                        value[1][0]['stride'] = (1, 2)
            # for key, value in model_kwargs.items():
            #    if isinstance(value, list):
            #        for elem in value:
            #            for item in elem:
            #                print(item)
            # print(model_kwargs, variant, pretrained)
        model = _create_model(model_kwargs=model_kwargs, variant=variant, pretrained=False)
    return model
